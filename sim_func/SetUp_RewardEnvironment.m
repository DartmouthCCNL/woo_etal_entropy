...
%% Create random reward environment 
    ...
    % simEnv.rev_address = rev_address; 
    % simEnv.blockType = blockType;
    
    % %% viz
    % figure(sd); clf; sgtitle(blockType+" "+max(RewProbs(:))*100+"/"+min(RewProbs(:))*100);
    % subplot(1,4,1); imagesc(simEnv.rewardprob_AB); title('rewards array Stim'); xticks([1 2]); xticklabels(["A","B"]);
    % yline(rev_address, "-r");    
    % subplot(1,4,2); imagesc(simEnv.rewardprob_LR); title('rewards array Loc'); xticks([1 2]); xticklabels(["Left","Right"]);
    % yline(rev_address, "-r");
    % subplot(1,4,3); imagesc(simEnv.stim_on_right); title('stim on right'); colorbar; 
    % yline(rev_address, "-r");
    % subplot(1,4,4); imagesc(simEnv.hr_side); title('hr side'); colorbar;
    % yline(rev_address, "-r");    
end